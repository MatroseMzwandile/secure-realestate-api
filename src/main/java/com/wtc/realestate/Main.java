...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        app.get("/listings", ctx -> ctx.json(listingDao.findAll()));

        app.get("/listings/{id}", ctx -> {
                int id = Integer.parseInt(ctx.pathParam("id"));
                Listing listing = listingDao.findById(id);
                if (listing == null) { ctx.status(404).json(Map.of("error","Listing not found")); }
                else { ctx.json(listing); }
        });
        ...
        app.post("/login", ctx -> {
            ...
            if ( realtor == null) {
                ctx.status(404).json(Map.of("error", "Invalid credentials"));
                return;
            }
        ...
        app.post("/listings", ctx -> {
            ...
            listing.setId(Integer.parseInt(String.valueOf(realtorId)));
            listing.setTitle(ctx.formParam("title"));
            listing.setDescription((ctx.formParam("description")));
            listing.setPrice(new BigDecimal(ctx.formParam("price")));
